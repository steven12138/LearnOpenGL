...
#define P_TEX_WIDTH  8    // Particle texture2D dimensions
#define P_TEX_HEIGHT 8
#define F_TEX_WIDTH  16   // Floor texture2D dimensions
#define F_TEX_HEIGHT 16

// Texture object IDs
GLuint particle_tex_id, floor_tex_id;

// Particle texture2D (a simple spot)
...
// Floor texture2D (your basic checkered floor)
...
static void draw_particles(GLFWwindow* window, double t, float dt)
{
    ...
    // situations). GL_T2F_C4UB_V3F means: 2 floats for texture2D coords,
    ...
    for (i = 0;  i < MAX_PARTICLES;  i++)
    {
        if (pptr->active)
        {
            ...
            // store texture2D coord and color information for each vertex).
    ...
    // glEnableClientState for vertex, texture2D coord and color arrays)
